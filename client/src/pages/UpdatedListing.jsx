...
const UpdatedListing = () => {
    ...
    const handleImageSubmit = (e) => {
        ...
        if (files.length > 0 && files.length + formData.imageUrls.length < 7) {
            ...
            Promise.all(promises).then((urls) => {
                ...
                setImageUploadError("image Upload size (2MB per one")
                setUploading(false)
            })
        } else {
            setImageUploadError("Can only upload 6 images max")
            setUploading(false)
        }
    }
    ...
    const storeImage = async (file) => {
        ...
        // A promise is essentially an improvement of callbacks 
        // that manage all asynchronous data activities
        // that will either be completed or declined
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if(+formData.regularPrice < +formData.discountedPrice) return setSubmitError("Discount must be smaller")
                ...
                <div className="flex flex-col flex-1 gap-4">
                    ...
                    <p className="text-red-700 text-sm">{imageUploadError && imageUploadError}</p>
                    {
                        formData.imageUrls.length > 0 && formData.imageUrls.map((url, index) => (
                            <div key={url} className='flex p-3 justify-between items-center border'>
                    ...
                    {submitError && <span className="text-red-700 text-sm">{submitError}</span>}
                </div>
            </form>
        </main>
    )
